...
contract EstateAgency { 
    ...
    function createEstate(uint size, string memory addressEstate, EstateType esType) public { 
        ...
        estates.push(Estate(size, addressEstate, msg.sender, esType, true, estates.length + 1)); 
        emit createdEstate(msg.sender, estates.length +1, block.timestamp, esType); 
    } 
    ...
    function createAduint(address buyer, uint amount, uint idEstate) public onlyEstateOwner(idEstate) isActiveEstate(idEstate) { 
        ...
        ads.push(Advertisement(msg.sender, buyer, amount * 1000000000000000000, idEstate, ads.length +1, block.timestamp, AdStatus.Opened)); 
